...
public class Products
{
    ...
    public int saveProduct()
    {
        ...
        if (this.idProduct == 0)
        {
            ...
            product.Add(this.ExpirationDate.ToString("yyyy:MM:dd hh:mm:ss"));
    ...
    /// <summary>
    /// Method to save the Sells that the Seller make.
    /// </summary>
    /// <param name="idCustomer">Int ID of the Customer</param>
    /// <param name="idSeller">Int ID of the Seller</param>
    /// <param name="Total">Int that is the total (money) of the sell</param>
    /// <param name="idProducts">Array of String with the IDs of the Sells to be saved</param>
    /// <param name="Amounts">Array of String with the Amounts of the Sells to be saved</param>
    public void saveSell(int idCustomer, int idSeller, Double Total, string[] idProducts, string[] Amounts)
    {
        ArrayList parameters = new ArrayList();
        DataTable result;
        int Amount = 0;
        ...
        {
            Amount += Convert.ToInt16(Amounts[i]);
        }

        parameters.Add(idCustomer);
        parameters.Add(idSeller);
        parameters.Add(Amount);
        parameters.Add(Total);
        parameters.Add(StartDate.ToString("yyyy:MM:dd hh:mm:ss"));

        result = aR.callProcedure("setSell", parameters);

        if(result != null && (Convert.ToInt16(result.Rows[0].ItemArray[0].ToString()) > 0))
        {
            int idSell = Convert.ToInt16(result.Rows[0].ItemArray[0].ToString());
            parameters.Clear();
            for (int i = 0; i < idProducts.Length; i++)
            {
                parameters.Add(idSell);
                parameters.Add(idProducts[i]);
                parameters.Add(Amounts[i]);
                aR.callProcedure("setProductsSell", parameters);
                parameters.Clear();
            }
        }
    }
    ...
    /// <summary>
    /// Method to obtain the last Sells of the table.
    /// </summary>
    /// <returns>Returns a DataTable with the last 5 rows of Sells</returns>
    public DataTable getLastSells()
    {
        return this.aR.callProcedure("getLastSells");
    }
}